...
Прежде чем запустить приложение необходимо создать исполняемый jar файл.  </p>
Исполняемый JAR (Java Archive) файл в Intellij IDEA используется для запуска приложения, написанного на языке программирования Java, в окружении, где установлено только окружение выполнения Java (JRE). </p>
...
Исполняемый JAR файл может быть запущен из командной строки с помощью команды "java -jar имя_файла.jar" или двойным щелчком мыши на файле в операционной системе, если ассоциация файлов JAR настроена для запуска через JRE.
